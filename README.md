...
核心文案为“每天30分钟，轻松准备入学”。页面保留毛绒风格板块图标、Edge / Microsoft 真人语音选择与语音开关。简介只在首次打开时展示，学习时不再常驻占用页面空间。
...
家长视图改为圆环任务达成入口，不再作为侧栏常驻。圆环显示学习任务完成比例，圆环下方显示当前可用星星数量。
...
每次答对题目增加 1 颗星星。星星可以在家长视图中兑换亲子绘本、周末小游戏、奖励贴纸等项目。兑换后会扣减可用星星，并写入兑换日志，方便家长查看孩子的奖励记录。
